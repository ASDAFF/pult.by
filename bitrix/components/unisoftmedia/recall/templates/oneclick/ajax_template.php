...
?>
	...
	<?if(strlen($arResult["OK_MESSAGE"]) <= 0 || $arParams['POPUP_FORM'] != 'Y'):?>
		<form action="<?=POST_FORM_ACTION_URI?>" method="POST">
			...
			<?if($arParams['USE_ONECLICK'] == 'Y'):?>
				<input type="hidden" name="oneclick" value="Y" />
				<input type="hidden" name="ELEMENT_ID" value="<?php echo $arParams['ELEMENT_ID'] ?>" />
				<?if($arParams['ONECLICK_ORDER'] == 'Y'):?>
			...
			<?if($arParams['USE_ONECLICK'] == 'Y'):?>
				<div class="form-group">
					...
					<input class="form-control" type="text" name="product_name" value="<?=$arParams["PRODUCT_NAME"]?>" disabled />
				</div>
			<?endif?>
			...
			<?if ($arParams['USER_CONSENT'] === 'Y')
        {
			...
			<button onclick="ga('send', 'event', 'button', 'click', 'oneclick');" class="btn btn-primary" type="submit" name="submit" <?if($arParams['POPUP_FORM'] == 'Y' || $arParams['MODE_AJAX'] == 'Y'):?>onclick="Recall.submit('<?php echo $recallId ?>', '<?php echo $arParams['USER_CONSENT'] ?>'); return false;"<?endif?> value="<?=Loc::getMessage("MFT_SUBMIT")?>"><?=Loc::getMessage("MFT_SUBMIT")?></button>
		</form>
	<?endif?>