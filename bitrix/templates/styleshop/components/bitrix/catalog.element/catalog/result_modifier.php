...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	\Unisoftmedia\Styleshop\Bitrix\CIBlockPriceTools::getTreePropertyValues($arSKUPropList, $arNeedValues);
	$arSKUPropIDs = array_keys($arSKUPropList);
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		{
			$arPic = CFile::ResizeImageGet($arOffer['PREVIEW_PICTURE']['ID'],
				array(
					"width" => 50,
					"height" => 70
				),
		...
		foreach ($arSKUPropIDs as $propkey => $strOneCode)
		{
			...
			if (isset($arOffer['DISPLAY_PROPERTIES'][$strOneCode]))
			{
				...
				{
					$skuDirectory = $arSKUPropList[$strOneCode]['VALUES'][$arSKUPropList[$strOneCode]['XML_MAP'][$arOffer['DISPLAY_PROPERTIES'][$strOneCode]['VALUE']]];
					$currentSku[$keyOffer][] = array(
						'ID' => $skuDirectory['ID'],
						'NAME' => $arSKUPropList[$strOneCode]['NAME'],
						'VALUE' => $skuDirectory['NAME'],
						'PICT' => $skuDirectory['PICT'],
						'PICT2' => !empty($arPic)? $arPic['src']: false,
						'USER_TYPE' => $arSKUPropList[$strOneCode]['USER_TYPE'],
						'SORT' => $arSKUPropList[$strOneCode]['SORT'],
						'XML_ID' => $skuDirectory['XML_ID'],
					...
					if(!empty($arPic))
					{
						$arSKUPropList[$strOneCode]['VALUES'][$intValue]['PICT2']['SRC'] = $arPic['src'];
						$arSKUPropList[$strOneCode]['VALUES'][$intValue]['PICT2']['WIDTH'] = $arPic['width'];
						$arSKUPropList[$strOneCode]['VALUES'][$intValue]['PICT2']['HEIGHT'] = $arPic['height'];
					}
				}
		...
		$arNewOffers[$keyOffer]['SKU'] = $currentSku[$keyOffer];
	}
...
/******************** DOCUMENTATIONS ************************/
if(is_array($arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE']) && !empty($arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE']) && 'F' == $arResult['PROPERTIES']['DOCUMENTATIONS']['PROPERTY_TYPE'])
{
    foreach($arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE'] as $k => $docsProp)
    {
        $rsFile = CFile::GetByID($docsProp);
        if($arFile = $rsFile->Fetch())
        {
            $arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE'][$k] = $arFile;
            $fileType = explode('.', $arFile['FILE_NAME']);
            ...
            $arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE'][$k]['TYPE'] = $type;
            $arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE'][$k]['SIZE'] = $size;
            $arResult['PROPERTIES']['DOCUMENTATIONS']['VALUE'][$k]['LINK_PATH'] = '/upload/'.$arFile['SUBDIR'].'/'.$arFile['FILE_NAME'];
        }
    }
}
/******************** /DOCUMENTATIONS ************************/
