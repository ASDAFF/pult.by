...
BX.Catalog.SetConstructor = (function()
{
	...
	SetConstructor.prototype.recountPrice = function()
	{
		...
		{
			BX.adjust(this.setOldPriceCont, { style: { display: '' }, html: BX.Currency.currencyFormat(sumOldPrice, this.currency, true) });
			this.setDiffPriceCont.innerHTML =  this.messages.CATALOG_SET_DISCOUNT_DIFF.replace("#PRICE#", BX.Currency.currencyFormat(sumDiffDiscountPrice, this.currency, true));
		}
		else
		{
			BX.adjust(this.setOldPriceCont, { style: { display: 'none' }, html: '' });
	...
	SetConstructor.prototype.addToBasket = function()
	{
		...
		BX.addClass(target, 'disabled');

		curText = target.innerHTML;
		loading = target.getAttribute("data-loading");
		added = target.getAttribute("data-added");
		...
		BX.ajax.post(
			...
			{
				BX.removeClass(target, 'disabled');
