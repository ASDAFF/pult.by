...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($ob = $res->GetNextElement())
	{
	   ...
	   if(CCatalogSKU::IsExistOffers($arElement["ID"],$arElement["IBLOCK_ID"]))
        {
            $arProps = $ob->GetProperties();
            if($arProps['MINIMUM_PRICE']['VALUE'])
            {
                $arElement["PRICES"][$arParams["PRICE_CODE"][0]]['PRINT_VALUE'] = CCurrencyLang::CurrencyFormat($arProps['MINIMUM_PRICE']['VALUE'],CCurrency::GetBaseCurrency(), true);
                $arElement["PRICES"][$arParams["PRICE_CODE"][0]]['MIN_PRICE'] = 'Y';
                $arElement["PRICES"][$arParams["PRICE_CODE"][0]]['CAN_ACCESS'] = 'Y';
                $arElement["PRICES"][$arParams["PRICE_CODE"][0]]['OFFERS'] = 'Y';
            }
        }
