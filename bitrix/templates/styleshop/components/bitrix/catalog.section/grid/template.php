...
?>
	...
	<div class="catalog-products-container">
		<div class="row">
			<div class="unproduct <?php echo $arParams['COLUMN'] ?>">
				<?foreach ($arResult['ITEMS'] as $key => $arItem)
				{
				   ...
				   $arJSParams = [
					...
					if('Y' == $arItem['PROPERTIES']['SALELEADER']['VALUE']) {
...
?>
					<article class="<?if($arParams['COLUMN'] == 'one_column'):?>col-xs-6 col-sm-4 col-md-3 col-lg-3<?else:?>col-xs-6 col-sm-4 col-md-4 col-lg-3<?endif?> unproduct-item js-item" id="<?php echo $strMainID ?>">
...
$uf_iblock_id = 12; //ID инфоблока
$uf_name = Array("UF_NAME"); //пользовательское поле UF_PAGE_LINK
...
if(CModule::IncludeModule("iblock")): //подключаем модуль инфоблок для работы с классом CIBlockSection
   $uf_arresult = CIBlockSection::GetList(Array("SORT"=>"­­ASC"), Array("IBLOCK_ID" => $uf_iblock_id, "ID" => $arResult['ID']), false, $uf_name);
   if($uf_value = $uf_arresult->GetNext()):
      if(strlen($uf_value["UF_NAME"]) > 0): //проверяем что поле заполнено
      $ar = $uf_value["UF_NAME"]; //подменяем ссылку и используем её в дальнейшем
      endif;
   endif;
endif;
?> 
...
<span class="stickerhit"<?php echo ('' == $stickerHit)? ' style="display: none;"' : '' ?>>
										<span class="text"><?php echo $arItem['PROPERTIES']['SALELEADER']['NAME']?></span>
									</span>
									</div>


									<h3><a title="<?php echo $arItem['NAME']?>" href="<?php echo $arItem['DETAIL_PAGE_URL']?>"><?echo $ar;?>   <?php echo $arItem['NAME']?></a></h3>
								</div>
