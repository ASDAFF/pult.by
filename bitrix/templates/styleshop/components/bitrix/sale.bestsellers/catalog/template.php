...
?>
		<div class="row">
			<div class="unproduct <?if($arParams['USE_CAROUSEL'] == 'Y'):?>owl-carousel Owlcarousel<?endif?>" <?if($arParams['USE_CAROUSEL'] == 'Y'):?>data-options="<?php echo CUtil::PhpToJSObject($sliderOptions, false, true); ?>"<?endif?>>
				...
				{
					if(!$arItem['CAN_BUY'] && !$arItem['OFFERS'][$arItem['OFFERS_SELECTED']]['CAN_BUY']) 
						continue;
...
?>
									...
									{?>
										<div class="compare"><a data-toggle="tooltip" data-placement="top" href="<?php echo $arItem['COMPARE_URL'] ?>" class="btn add2compare add2compare_<?php echo $arItem['ID']?>" data-compareurl="<?php echo SITE_DIR?>catalog/compare/" title="<?php echo $arParams['MESS_BTN_COMPARE']?>" id="<?php echo $arJSParams['VISUAL']['COMPARE_ID']?>" data-compare="<?php echo $arItem['ID']?>">
