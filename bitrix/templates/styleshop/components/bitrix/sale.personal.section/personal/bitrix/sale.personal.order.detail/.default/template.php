...
<?else:?>
	...
	<div class="bx_order_list">
		...
		<table class="bx_order_list_table table">
			<tbody>
				...
				<tr class="no_border"><td><br></td><td></td></tr>

				<?if(intval($arResult["USER_ID"])):?>

					<tr class="no_border">
						<td colspan="2"><h3><?=GetMessage('SPOD_ACCOUNT_DATA')?></h3></td>
					</tr>
				...
				<tr class="no_border">
					<td colspan="2"><h3><?=GetMessage('SPOD_ORDER_PROPERTIES')?></h3></td>
				</tr>
				<tr>
				...
				<?if (isset($arResult["ORDER_PROPS"])):?>
					<?foreach($arResult["ORDER_PROPS"] as $prop):?>
						...
						<?if($prop["SHOW_GROUP_NAME"] == "Y"):?>
							...
							<tr class="no_border"><td><br></td><td></td></tr>
							<tr class="no_border">
								<td colspan="2"><h3><?=$prop["GROUP_NAME"]?></h3></td>
							</tr>
				...
				<tr class="no_border"><td><br></td><td></td></tr>
				<tr class="no_border">
					<td colspan="2"><h3><?=GetMessage("SPOD_ORDER_PAYMENT")?></h3></td>
				</tr>
							...
							<td colspan="2">
								<?
									...
									elseif (array_key_exists('BUFFERED_OUTPUT', $payment))
										echo str_replace('type="submit"', 'type="submit" class="btn btn-primary"', $payment['BUFFERED_OUTPUT']);
								?>
							</td>
						</tr>
					<?endif?>
	...
	<div class="bx_ordercart">
						...
						<tr>
							...
							<td class="itemphoto">
								...
								<?if($prod['PICTURE']['SRC']):?>
									...
									<img class="img-thumbnail" src="<?=$prod['PICTURE']['SRC']?>" width="<?=$prod['PICTURE']['WIDTH']?>" height="<?=$prod['PICTURE']['HEIGHT']?>" alt="<?=$prod['NAME']?>" />
							...
							<td class="item">
								...
								?>
									<?if($actuallyHasProps):?>
										<ul class="sku_prop_list list-inline">
							...
							<td class="content_price<?=(isset($prod['DISCOUNT_PRICE_PERCENT_FORMATED'])? ' old': '')?>" data-label="<?=GetMessage('SPOD_PRICE')?>"><div class="price"><?=$prod["PRICE_FORMATED"]?></div></td>

							<?if($arResult['HAS_DISCOUNT']):?>
								<td class="custom price" data-label="<?=GetMessage('SPOD_DISCOUNT')?>"><?=(isset($prod['DISCOUNT_PRICE_PERCENT_FORMATED']))? $prod["DISCOUNT_PRICE_PERCENT_FORMATED"]: '-'?></td>
							<?endif?>

							<td class="custom amount" data-label="<?=GetMessage('SPOD_PRICETYPE')?>"><?=htmlspecialcharsEx($prod["NOTES"])?></td>
							<td data-label="<?=GetMessage('SPOD_QUANTITY')?>">
								<span class="fm"><?=$prod["QUANTITY"]?>
			...
			<div class="col-xs12">
				<table class="bx_ordercart_order_sum">
					<tbody>
						...
						<tr>
							<td class="custom_t1 fwb"><b><?=GetMessage('SPOD_SUMMARY')?>:</b></td>
							<td class="custom_t2 fwb"><b><?=$arResult["PRICE_FORMATED"]?></b></td>
						</tr>
					</tbody>
				</table>
			</div>
		...
		<table class="bx_control_table" style="width: 100%;">
			<tr>
				<td>  <a href="<?=$arResult["URL_TO_LIST"]?>" class="btn btn-default"><?=GetMessage('SPOD_GO_BACK')?></a></td>
			</tr>
		</table>
