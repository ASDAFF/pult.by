		...
		if (createNewItem)
		{
			...
			for (i = 0; i < arColumns.length; i++)
			{
				switch (arColumns[i])
				{
					...
					case 'NAME':
						...
						if (arItem['DETAIL_PAGE_URL'].length > 0)
							cellItemHTML += '<h4 class="bx_ordercart_itemtitle"><a href="' + arItem['DETAIL_PAGE_URL'] + '">' + arItem['NAME'] + '</a></h4>';
						else
							cellItemHTML += '<h4 class="bx_ordercart_itemtitle">' + arItem['NAME'] + '</h4>';

						cellItemHTML += '<ul class="bx_ordercart_itemart list-inline clearfix">';
						...
						if (bShowPropsColumn)
						{
							for (j = 0; j < arItem['PROPS'].length; j++)
							{
								...
								cellItemHTML += '<li>';
								cellItemHTML += '<span class="bx_ordercart_itemart-name">'+BX.util.htmlspecialchars(val['NAME'])+'</span>';
								cellItemHTML += '</li>';
								cellItemHTML += '<li>';
								cellItemHTML += '<span class="bx_ordercart_itemart-value">'+val["VALUE"]+'</span>';
								cellItemHTML += '</li>';
							}
						}
						cellItemHTML += '</ul>';
						...
						if (arItem.SKU_DATA)
						{
							...
							for (propId in arItem.SKU_DATA)
							{
								if (arItem.SKU_DATA.hasOwnProperty(propId))
								{
									...
									if (bIsImageProperty)
									{
										cellItemHTML += '<div class="block-sku">';
										cellItemHTML += '<div class="block-sku-detail-sku">';
										cellItemHTML += '<span class="block-sku-detail-sku-name">' + arProp['NAME'] + '</span>';
										cellItemHTML += '<div class="block-sku-detail-sku-container">';
										cellItemHTML += '<div class="bx_scu">';

										cellItemHTML += '<ul id="prop_' + arProp['CODE'] + '_' + arItem['ID'] + '" class="block-sku-detail-sku-container-list">';
									...
									else // not image
									{
										...
										for (valueId in arProp['VALUES'])
										{
											...
											cellItemHTML += '<li \
																class="sku_prop ' + selected + '"\
																data-sku-selector="Y" \
																data-value-id="' + (arSkuValue['XML_ID'] ? arSkuValue['XML_ID'] : BX.util.htmlspecialchars(arSkuValue['NAME'])) + '" \
																data-sku-name="' + BX.util.htmlspecialchars(arSkuValue['NAME']) + '" \
																data-element="' + arItem['ID'] + '"\
																data-property="' + arProp['CODE'] + '"\
																>\
					...
					if (basketMessage['QUANTITY']) {
						oCellQuantity.setAttribute('data-label', basketMessage['QUANTITY']);
					}
						
					oCellQuantityHTML += '<span>' + getColumnName(res, arColumns[i]) + ':</span>';

					oCellQuantityHTML += '<div class="centered">';
						oCellQuantityHTML += '<div class="form-inline">';
					oCellQuantityHTML += '<div class="quantity form-group">';
					oCellQuantityHTML += ' <div class="input-group">';

					if (ratio != 0
						&& ratio != ''
						) // if not Set parent, show quantity control
					{
						oCellQuantityHTML += '<div class="input-group-btn">\
							<button type="button" class="btn btn-sm btn-default minus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');">-</button>\
						</div>';
					}

					oCellQuantityHTML += '<input class="form-control qty min text-center" type="text" size="3" id="QUANTITY_INPUT_' + arItem['ID'] + '"\
					...
					{
						oCellQuantityHTML += '<div class="input-group-btn">\
							<button type="button" class="btn btn-sm btn-default plus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'up\', ' + bUseFloatQuantity + ');">+</button>\
						</div>';
					}
					...
					if (arItem.hasOwnProperty('MEASURE_TEXT') && arItem['MEASURE_TEXT'].length > 0)
						oCellQuantityHTML += '<span class="measure_name">' + arItem['MEASURE_TEXT'] + '</span>';
					...
					case 'PRICE':
						...
						oCellPrice.setAttribute('class', 'content_price');
						if (basketMessage['PRICE']) {
							oCellPrice.setAttribute('data-label', basketMessage['PRICE']);
						}

						oCellPrice.innerHTML = '<div class="price" id="current_price_' + arItem['ID'] + '">' + arItem['PRICE_FORMATED'] + '</div>' +
					...
					case 'DISCOUNT':
						...
						if (basketMessage['DISCOUNT']) {
							oCellDiscount.setAttribute('data-label', basketMessage['DISCOUNT']);
						}
					...
					case 'WEIGHT':
						...
						if (basketMessage['WEIGHT']) {
							oCellWeight.setAttribute('data-label', basketMessage['WEIGHT']);
						}
					...
					default:
						...
						if (arColumns[i] == 'SUM') {
							oCellCustom.setAttribute('class', 'custom sum content_price');
						} else {
							oCellCustom.setAttribute('class', 'custom');
						}
						

						if (basketMessage[basketMessage[arColumns[i]]]) {
							oCellCustom.setAttribute('data-label', basketMessage[basketMessage[arColumns[i]]]);
						}
			...
			if (bShowDeleteColumn || bShowDelayColumn)
			{
				...
				if (bShowDeleteColumn)
					oCellControl.innerHTML = '<div><a title="' + basketJSParams['SALE_DELETE'] + '" class="delete" href="' + basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) +'"></a></div>';

				if (bShowDelayColumn)
					oCellControl.innerHTML += '<div><a title="' + basketJSParams['SALE_DELAY'] + '" class="delay" href="' + basketJSParams['DELAY_URL'].replace('#ID#', arItem['ID']) + '"></a></div>';
			}
...
function skuPropClickHandler()
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		all_sku_props = BX.findChildren(BX(basketItemId), {tagName: 'ul', className: 'block-sku-detail-sku-container-list'}, true);
		if (!!all_sku_props && all_sku_props.length > 0)
		{
			for (i = 0; all_sku_props.length > i; i++)
			{
				...
				{
					sku_prop_value = BX.findChildren(BX(all_sku_props[i].id), {tagName: 'li', className: 'active'}, true);
