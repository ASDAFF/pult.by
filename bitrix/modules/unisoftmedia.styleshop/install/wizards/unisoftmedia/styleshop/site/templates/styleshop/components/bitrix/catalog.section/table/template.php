...
if (!empty($arResult['ITEMS']))
{
    ...
    if(!empty($arResult['DESCRIPTION'])){
        ?><div class="category-description-block hidden-xs"><?=$arResult['DESCRIPTION']?></div><?
    }
	$this->EndViewTarget();
	
	$this->SetViewTarget("picture");
	if(!empty($arResult['PICTURE'])){
		?><div class="category-image-block"><img src="<?=$arResult["PICTURE"]['SRC']?>" title="<?=$arResult['NAME']?>" alt="<?=$arResult['NAME']?>" /></div><?
	}
	$this->EndViewTarget();
	
	if('Y' === $arParams['SEARCH_PAGE'])
	{
        $this->SetViewTarget("search_page_count_element");
            $search_page_count = count($arResult['ITEMS']);
            if((0 < $search_page_count) && ($search_page_count < 2)){
                echo $search_page_count.' '.Loc::getMessage('CT_BST_SEARCH_RESULTS1');
            } else if((1 < $search_page_count) && ($search_page_count < 5)){
                echo $search_page_count.' '.Loc::getMessage('CT_BST_SEARCH_RESULTS2');
            } else if($search_page_count > 5){
                echo $search_page_count.' '.Loc::getMessage('CT_BST_SEARCH_RESULTS3');
            }
        $this->EndViewTarget();
    }
...
?>
	<div class="catalog-products-container">
		<div class="row">
			<div class="unproduct_list <?php echo $arParams['COLUMN'] ?>">
...
?>
					<article class="col-xs-12 unproduct-item js-item" id="<?php echo $strMainID ?>">
						<div class="unproduct-container row">
							...
							<?/********************** sticker ************************/?>
								...
								if($arParams['QUICK_LOOK_PARAMS'] == 'QUICK_LOOK_BUTTON')
								{?>
							...
							<div class="unproduct-bottom-container col-xs-8 col-sm-9 col-md-9 col-lg-9">
								<div class="col-sm-8 col-md-8 col-lg-9">
									...
									<div class="desc hidden-xs"><?php echo $arItem['PREVIEW_TEXT'] ?></div>
									<div class="hidden-xs">
										<?/********************************* SKU *************************************/
											$arSkuProps = array();
											if (!empty($arResult['SKU_PROPS'])) {
								...
								<div class="block_list col-sm-3 col-md-4 col-lg-3">
									<div class="content_price">
										...
										{?>
											<span id="<?php echo $arJSParams['VISUAL']['OLD_PRICE_ID']?>" class="hidden-xs old-price"<?php echo ($minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])? '' : ' style="display: none;"' ?>><?php echo $minPrice['PRINT_VALUE']; ?></span>
										<?}?>
								...
								unset($minPrice);
?>
									<div class="button-container<?if($OFFERS):?> hidden-sm hidden-md hidden-lg<?endif?>">
										<a class="btn btn-primary block-top" role="button" title="<?php echo $arParams['MESS_BTN_DETAIL']?>" href="<?php echo $arItem["DETAIL_PAGE_URL"]?>">
											<span class="text"><?php echo $arParams['MESS_BTN_DETAIL']?></span>
										</a>
									</div>
									<div class="button-container<?if($OFFERS):?> hidden-xs<?endif?>">
									<?/************************ ADD TO CART *************************/
										...
										if(!$OFFERS)
										{
											if($arItem['CAN_BUY']) {?>
												<a
												class="btn un_buttoncart btn-primary add2basket add2basket_<?php echo $arItem['ID'] ?>"
...
?>
													...
													class="btn offers add2basket add2basket_<?php echo $arItem['OFFERS'][$arItem['OFFERS_SELECTED']]['ID'] ?> un_buttoncart btn-primary"
													data-options="<?php echo CUtil::PhpToJSObject($arSettingsBasket, false, true); ?>"
