...
?>
						...
						{?>
							<span id="<?php echo $arJSParams['VISUAL']['OLD_PRICE_ID']?>" class="old-price"<?php echo ($boolDiscountShow)? '' : ' style="display: none;"' ?>><? echo($boolDiscountShow ? $minPrice['PRINT_VALUE'] : ''); ?></span>
						<?}?>
							<span id="<?php echo $arJSParams['VISUAL']['PRICE_ID'] ?>" class="price<?php echo ($boolDiscountShow)? ' old' : ' ' ?>"><?php echo $minPrice['PRINT_DISCOUNT_VALUE'] ?></span>
					</div>
...
?>
				<a href="#" <?if($arParams['AJAX_QUICKVIEW'] && $OFFERS):?>style="border-bottom: 0;cursor: default;"<?endif?> class="dropdown-toggle" data-toggle="dropdown" role="button"><?php echo Loc::getmessage('CATALOG_STOCK') ?></a>
