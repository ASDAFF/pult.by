...
if(!isset($arParams['MAX_WIDTH']))
	$arParams['MAX_WIDTH'] = 70;
if(!isset($arParams['MAX_HEIGHT']))
	$arParams['MAX_HEIGHT'] = 70;
			...
			if (isset($_REQUEST[$arParams['PRODUCT_ID_VARIABLE']]))
			{
				...
				if ($productID > 0 && !isset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$productID]))
				{
					...
					if ($found)
					{
						...
						if(isset($arElement['DETAIL_PICTURE']) && intval($arElement['DETAIL_PICTURE']) > 0)
						{
							$idPict = intval($arElement['DETAIL_PICTURE']);
							$arPic = CFile::ResizeImageGet($idPict,
								array(
						...
						if(isset($arElement['PREVIEW_PICTURE']) && intval($arElement['PREVIEW_PICTURE']) > 0)
						{
							$idPict = intval($arElement['PREVIEW_PICTURE']);
							$arPic = CFile::ResizeImageGet($idPict,
								array(
									"width" => $arParams['MAX_WIDTH'],
									"height" => $arParams['MAX_HEIGHT']
						...
						$_SESSION[$arParams['NAME']][$arParams['IBLOCK_ID']]['ITEMS'][$productID] = array(
							...
							'~DETAIL_PAGE_URL' => $arElement['~DETAIL_PAGE_URL'],
							'DETAIL_PICTURE' => (isset($arElement['DETAIL_PICTURE']) && !empty($arElement['DETAIL_PICTURE']))? $arElement['DETAIL_PICTURE'] : false,
							'PREVIEW_PICTURE' => (isset($arElement['PREVIEW_PICTURE']) && !empty($arElement['PREVIEW_PICTURE']))? $arElement['PREVIEW_PICTURE'] : false,
