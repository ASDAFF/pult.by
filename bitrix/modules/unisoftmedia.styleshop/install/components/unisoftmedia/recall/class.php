...
class RecallComponent extends CBitrixComponent
{
    ...
    public function onPrepareComponentParams($params)
    {
        ...
        $this->isOneClick = ($params['USE_ONECLICK'] == 'Y');
        ...
        $this->obtainUserConsentInfo($params);
        ...
        if(empty($this->arResult['ERROR_MESSAGE']))
        {
            ...
            if ($this->isPhone && strlen($_SESSION['MF_PHONE']) > 0) {
                ...
                if($this->user()->IsAuthorized()) {
                    ...
                    $arUser = $rsUser->Fetch();
                    $this->arResult['AUTHOR_PHONE'] = $arUser['PERSONAL_PHONE'];
                }
    ...
    protected function obtainUserConsentInfo($params)
    {
        $this->arResult['USER_CONSENT_PROPERTY_DATA'] = [];
        foreach ($params['INCLUDE_FIELDS'] as $field) {
            $this->arResult['USER_CONSENT_PROPERTY_DATA'][] = Loc::getMessage("MFT_{$field}");
        }
        $this->arResult['USER_CONSENT_PROPERTY_DATA'] = array_filter($this->arResult['USER_CONSENT_PROPERTY_DATA'], function($item) {
            return !empty($item);
        });
    }
