...
use Unisoftmedia\Styleshop\Library\SimpleHtmlDomNode;
 ...
 */
class SimpleHtmlDom extends SimpleConst
{
    public $root = null;
    public $nodes = array();
    ...
    public function __construct($str=null, $lowercase=true, $forceTagsClosed=true, $target_charset=DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=parent::DEFAULT_BR_TEXT, $defaultSpanText=parent::DEFAULT_SPAN_TEXT)
    {
        if ($str)
        {
    ...
    public function load($str, $lowercase=true, $stripRN=true, $defaultBRText=parent::DEFAULT_BR_TEXT, $defaultSpanText=parent::DEFAULT_SPAN_TEXT)
    {
        global $debugObject;
        ...
        while ($this->parse());
        // end
        $this->root->_[parent::HDOM_INFO_END] = $this->cursor;
        $this->parse_charset();
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=parent::DEFAULT_BR_TEXT, $defaultSpanText=parent::DEFAULT_SPAN_TEXT)
    {
        $this->clear();
        ...
        $this->root->tag = 'root';
        $this->root->_[parent::HDOM_INFO_BEGIN] = -1;
        $this->root->nodetype = parent::HDOM_TYPE_ROOT;
    ...
    protected function parse()
    {
        ...
        ++$this->cursor;
        $node->_[parent::HDOM_INFO_TEXT] = $s;
    ...
    protected function read_tag()
    {
        ...
        if ($this->char==='/')
        {
            ...
            if ($parent_lower!==$tag_lower)
            {
                ...
                {
                    $this->parent->_[parent::HDOM_INFO_END] = 0;
                    ...
                    if (strtolower($this->parent->tag)!==$tag_lower) {
                        ...
                        $this->parent->_[parent::HDOM_INFO_END] = $this->cursor;
        ...
        $node = new SimpleHtmlDomNode($this);
        $node->_[parent::HDOM_INFO_BEGIN] = $this->cursor;
        ++$this->cursor;
        ...
        if (isset($tag[0]) && $tag[0]==='!') {
            $node->_[parent::HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

            if (isset($tag[2]) && $tag[1]==='-' && $tag[2]==='-') {
                $node->nodetype = parent::HDOM_TYPE_COMMENT;
                $node->tag = 'comment';
            } else {
                $node->nodetype = parent::HDOM_TYPE_UNKNOWN;
                $node->tag = 'unknown';
            }
            if ($this->char==='>') $node->_[parent::HDOM_INFO_TEXT].='>';
        ...
        if ($pos=strpos($tag, '<')!==false) {
            $tag = '<' . substr($tag, 0, -1);
            $node->_[parent::HDOM_INFO_TEXT] = $tag;
            $this->link_nodes($node, false);
        ...
        if (!preg_match("/^[\w-:]+$/", $tag)) {
            $node->_[parent::HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
            if ($this->char==='<') {
                $this->link_nodes($node, false);
                return true;
            }
        ...
        // begin tag
        $node->nodetype = parent::HDOM_TYPE_ELEMENT;
        $tag_lower = strtolower($tag);
        ...
        do
        {
            ...
            if ($this->pos>=$this->size-1 && $this->char!=='>') {
                $node->nodetype = parent::HDOM_TYPE_TEXT;
                $node->_[parent::HDOM_INFO_END] = 0;
                $node->_[parent::HDOM_INFO_TEXT] = '<'.$tag . $space[0] . $name;
                $node->tag = 'text';
                $this->link_nodes($node, false);
                return true;
            }
            ...
            if ($this->doc[$this->pos-1]=='<') {
                ...
                $node->attr = array();
                $node->_[parent::HDOM_INFO_END] = 0;
                $node->_[parent::HDOM_INFO_TEXT] = substr($this->doc, $begin_tag_pos, $this->pos-$begin_tag_pos-1);
                $this->pos -= 2;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $this->link_nodes($node, false);
                return true;
            ...
            if ($name!=='/' && $name!=='') {
                ...
                if ($this->char==='=') {
                    ...
                    $node->_[parent::HDOM_INFO_QUOTE][] = parent::HDOM_QUOTE_NO;
                    $node->attr[$name] = true;
                    if ($this->char!='>') $this->char = $this->doc[--$this->pos]; // prev
                }
                $node->_[parent::HDOM_INFO_SPACE][] = $space;
                $space = array($this->copy_skip($this->token_blank), '', '');
            }
            else
                break;
        ...
        $this->link_nodes($node, true);
        $node->_[parent::HDOM_INFO_ENDSPACE] = $space[0];

        // check self closing
        if ($this->copy_until_char_escape('>')==='/')
        {
            $node->_[parent::HDOM_INFO_ENDSPACE] .= '/';
            $node->_[parent::HDOM_INFO_END] = 0;
        }
        else
        {
        ...
        if ($node->tag == "br")
        {
            $node->_[parent::HDOM_INFO_INNER] = $this->default_br_text;
        }
    ...
    protected function parse_attr($node, $name, &$space)
    {
        ...
        switch ($this->char) {
            case '"':
                $node->_[parent::HDOM_INFO_QUOTE][] = parent::HDOM_QUOTE_DOUBLE;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $node->attr[$name] = $this->restore_noise($this->copy_until_char_escape('"'));
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                break;
            case '\'':
                $node->_[parent::HDOM_INFO_QUOTE][] = parent::HDOM_QUOTE_SINGLE;
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                $node->attr[$name] = $this->restore_noise($this->copy_until_char_escape('\''));
                $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
                break;
