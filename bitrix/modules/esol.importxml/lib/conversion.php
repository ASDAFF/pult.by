...
class Conversion
{
	...
	public function __construct($ie=false, $iblockId=0, $fieldSettings = array(), $type='element')
	{
		...
		$prefixPattern = '/^(OFFER_)?('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+$/';
		$prefixPattern2 = '/#((OFFER_)?('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+)#/';
	...
	public function ApplyExtraConversions($fKey, $val, $ID)
	{
		...
		if(isset($fs[$fKey]) && isset($fs[$fKey]['EXTRA_CONVERSION']) && is_array($fs[$fKey]['EXTRA_CONVERSION']))
		{
			...
			if($this->isSection)
			{
				...
				$prefixPattern2 = '/#(('.implode('|', $this->fieldSectPrefixes).')[A-Z0-9_]+)#/';
			}
			else
			{
	...
	public function ConversionExtraReplaceValues($m)
	{
		...
		$key = substr($m[0], 1, -1);
		if(isset($arFields[$key])) return $arFields[$key];
		else return '';
	}
