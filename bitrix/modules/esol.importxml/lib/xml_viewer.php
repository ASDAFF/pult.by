		...
		while(($isRead || $xml->read()) && $endTime-$beginTime < $maxTime) 
		{
			...
			if($xml->nodeType == \XMLReader::ELEMENT) 
			{
				...
				if(!isset($arXPaths[$xPath]))
				{
					...
					$nsPrefix = '';
					if($curNamespace && strpos($curName, ':')!==false)
					{
	...
	public function ShowXmlTag($arStruct)
	{
		foreach($arStruct as $k=>$v)
		{
			...
			if(is_array($v) && !empty($v['@attributes']))
			{
				...
				{
					echo ' '.$k2.'="<span class="esol_ix_str_value" data-attr="'.htmlspecialcharsex($k2).'"><span class="esol_ix_str_value_val">'.$this->GetShowVal($v2).'</span></span>"';
				}
				unset($v['@attributes']);
			}
			echo '&gt;';
			...
			{
				echo '<span class="esol_ix_str_value"><span class="esol_ix_str_value_val">'.$this->GetShowVal((is_array($v) && isset($v['@value'])) ? $v['@value'] : '').'</span></span>';
			}
			if(is_array($v) && isset($v['@value'])) 
			{
				unset($v['@value']);
