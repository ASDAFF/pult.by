...
if(CModule::IncludeModule('catalog'))
{
	...
	if($arCatalog = $dbRes->Fetch())
	{
		...
		{
			$arDefaultCatFields["ICAT_STORE".$arStore["ID"]."_AMOUNT"] = array('NAME' => GetMessage("ESOL_IX_LIST_QUANTITY_STORE").' "'.(strlen($arStore["TITLE"]) > 0 ? $arStore["TITLE"] : $arStore["ADDRESS"]).'"');
		}
		
		$dbRes = CCatalogGroup::GetList(array("SORT" => "ASC"));
		while($arPriceType = $dbRes->Fetch())
		{
			$arDefaultCatFields["ICAT_PRICE".$arPriceType["ID"]."_PRICE"] = array('NAME' => GetMessage("ESOL_IX_LIST_PRICE").' "'.($arPriceType["NAME_LANG"] ? $arPriceType["NAME_LANG"] : $arPriceType["NAME"]).'"');
		}
	}
}
