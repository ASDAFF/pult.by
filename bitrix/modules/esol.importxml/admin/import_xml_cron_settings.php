...
define("ESOL_IX_PATH2EXPORTS", "/bitrix/php_interface/include/esol.importxml/");
...
if ($_REQUEST["action"]=="save")
{
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='add')
	{
		...
		if (strlen($PROFILE_ID) > 0)
		{
			...
			$execFile = $_SERVER["DOCUMENT_ROOT"].ESOL_IX_PATH2EXPORTS."cron_frame.php";
			$logFile = $_SERVER["DOCUMENT_ROOT"].ESOL_IX_PATH2EXPORTS."logs/".$PROFILE_ID.".txt";
			$cfg_subdata = $strTime.$agent_php_path." -f ".$execFile." ".$PROFILE_ID." >".$logFile."\n";
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='delete')
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/crontab/crontab.cfg"))
		{
			...
			$cfg_data = preg_replace("#^.*?".preg_quote(ESOL_IX_PATH2EXPORTS)."cron_frame.php +".$PROFILE_ID." *>.*?$#im", "", $cfg_data);
