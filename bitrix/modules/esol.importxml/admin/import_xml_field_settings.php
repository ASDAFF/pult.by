...
//CJSCore::Init(array('esol_importxml'));
		...
		<?if($field=="IE_SECTION_PATH"){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					eval('$val = $P'.$fNameEval.';');
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsex($val)?>" placeholder="<?echo GetMessage("ESOL_IX_SETTINGS_SECTION_PATH_SEPARATOR_PLACEHOLDER");?>">
				</td>
			</tr>
			<tr>
		...
		<?if($bMultipleProp || $bMultipleField){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<input type="text" id="multiple_separator" name="<?=$fName2?>" value="<?=htmlspecialcharsex($val2)?>" placeholder="<?echo GetMessage("ESOL_IX_SETTINGS_MULTIPLE_SEPARATOR_PLACEHOLDER");?>" <?=($val!='Y' ? 'style="display: none"' : '')?>>
				</td>
			</tr>
		<?}?>
		<?if($bMultipleProp){?>
		...
		<tr>
			<td class="esol-ix-settings-margin-container" colspan="2">
				...
				foreach($arVals as $k=>$v)
				{
					...
					{
						$cellsOptions .= '<option value="{'.htmlspecialcharsex($k2).'}"'.($v['CELL']=='{'.$k2.'}' ? ' selected' : '').'>'.$v2.'</option>';
					}
					...
					<div id="settings_margins">
						...
						?>
							<div class="esol-ix-settings-margin" style="display: <?=($showMargin ? 'block' : 'none')?>;">
								...
								<?echo GetMessage("ESOL_IX_SETTINGS_AT_PRICE"); ?> <?echo GetMessage("ESOL_IX_SETTINGS_FROM"); ?> <input type="text" name="<?=$fName?>[PRICE_FROM][]" value="<?=htmlspecialcharsex($v['PRICE_FROM'])?>">
								<?echo GetMessage("ESOL_IX_SETTINGS_TO"); ?> <input type="text" name="<?=$fName?>[PRICE_TO][]" value="<?=htmlspecialcharsex($v['PRICE_TO'])?>">
			...
			<tr>
				...
				>
					...
					<?echo GetMessage("ESOL_IX_PICTURE_WATERMARK_FILE")?>:&nbsp;<input
						...
						size="35"
					>&nbsp;<input type="button" value="..." onClick="BtnClick<?echo strtr(htmlspecialcharsex($_GET['field_name']), array('['=>'_', ']'=>'_'))?>()">
				</div>
				<div class="adm-list-item"
