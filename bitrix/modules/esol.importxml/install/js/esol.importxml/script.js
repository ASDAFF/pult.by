var EIXPreview = {
	...
	ShowBaseElements: function(link)
	{
		...
		menuItems.push({
			TEXT: BX.message("ESOL_IX_GROUP_PROPERTY"),
			ONCLICK: 'EIXPreview.SetBaseElement("PROPERTY")'
		});
		if($('#esol_ix_xml_wrap select[name="offer_fields"]').length > 0)
		{
			menuItems.push({
				TEXT: BX.message("ESOL_IX_GROUP_OFFER"),
				ONCLICK: 'EIXPreview.SetBaseElement("OFFER")'
			});
		}
		menuItems.push({
			TEXT: BX.message("ESOL_IX_GROUP_SECTION"),
			ONCLICK: 'EIXPreview.SetBaseElement("SECTION")'
		});
		menuItems.push({
			TEXT: BX.message("ESOL_IX_GROUP_SUBSECTION"),
			ONCLICK: 'EIXPreview.SetBaseElement("SUBSECTION")'
		});
		...
		if((typeof option == 'object') && option.value)
		{
			...
			else
			{
				...
				if(!valObjVal.hasClass('esol_ix_str_value_val_selected'))
				{
					...
					var addLink = $('<a href="javascript:void(0)" class="esol_ix_str_value_add" title="'+BX.message("ESOL_IX_ADD_FIELD")+'"></a>');
	...
	ShowFieldSettings: function(e, btn)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/esol_import_xml_field_settings.php?field='+val+'&field_name='+name+'&index='+index+'&PROFILE_ID='+form.PROFILE_ID.value,
			'width':'900',
			'height':'400',
			'resizable':true,
...
var EProfile = {
	...
	ShowCron: function()
	{
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/esol_import_xml_cron_settings.php',
			'width':'800',
			'height':'350',
			'resizable':true});
	...
	OpenMissignElementFields: function(link)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/esol_import_xml_missignelem_fields.php?IBLOCK_ID='+iblockId+'&INPUT_ID='+input.attr('id')+'&OLDDEFAULTS='+input.val(),
			'width':'800',
			'height':'400',
			'resizable':true
		};
	...
	ShowEmailForm: function()
	{
		...
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/esol_import_xml_source_email.php?PROFILE_ID='+pid,
			'content_post': post,
			'width':'900',
			'height':'450',
			'resizable':true});
	...
	ShowFileAuthForm: function()
	{
		...
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/esol_import_xml_source_linkauth.php?PROFILE_ID='+pid,
			'content_post': post,
			'width':'900',
			'height':'450',
			'resizable':true});
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/esol.importxml/'+this.pid+'.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
...
var EHelper = {
	ShowHelp: function(index)
	{
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/esol_import_xml_popup_help.php',
			'width':'900',
			'height':'450',
			'resizable':true});
	...
	if(typeof anySelect.chosen != 'function')
	{
		...
		if(jQuerySrc)
		{
			$.getScript(jQuerySrc, function(){
				$.getScript('/bitrix/js/esol.importxml/chosen/chosen.jquery.min.js');
			});
		}
	}
	/*/Bug fix with excess jquery*/
