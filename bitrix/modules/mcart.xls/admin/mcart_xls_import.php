...
//========================================== profile ============================================
if ($_REQUEST["del_prof"]):			//Get profile.
...
<?elseif ($_REQUEST["del_prof_real"]):		//Delete profile.
?>
	<?
...
<?else:?>	<!-- No profile -->
	...
	//=========================== verification file ===============================
	if (empty($inputFileName) && empty($_GET['xls_file_name']))
	{
	...
	if (empty($errMess))
	{
		...
		{
			$inputFileName = $_GET['xls_file_name'];
		}
	    ...
	    try {
		    ...
		    $inputFileType = PHPExcel_IOFactory::identify($inputFileName);	//Get the file type
		    COption::SetOptionString("mcart.xls", "format", $inputFileType);	//Save file type

		    // ============== Create Reader  if file type - CSV =======================
		    ...
		    // ============== Create Reader  if file type - Excel2007 =======================
	    ...
	    <?
	    //================================== Get list type IBlock for Excel2007 ======================================
	    if($inputFileType=='Excel2007')
	    {
		$arrTypes = array();
		...
		<h4><?=GetMessage("MCART_ROWS_END")?></h4>
		    ...
		    <input type="radio" name="rows_end_label" class="lastrownumber" value="lastrownumber" onclick="ShowInput()" ><?=GetMessage('XLS_MCART_ROW_END_NUMBER')?><br />
		    <input type="text" class="last_row_num" name="last_row_num" style="display: none; margin-top:10px; margin-bottom:10px;" size="3" />
		...
		<?
		$MCART_IS_SKU = false;
		// If the modules is installed - 'catalog' and 'sale'
		if ((CModule::IncludeModule('catalog'))&&(CModule::IncludeModule('sale'))):
		    $MCART_IS_SKU = true;?>
		    <h4>
			<input type="checkbox" class="add_sku"  name="add_sku" value="Y" <? if ($ARR_REAL_PROFILE['need_offer']):?> checked <? endif;?>onclick="ShowSKU(this)">
			<?=GetMessage("XLS_SELECT_SKU_IBLOCK")?>
		    </h4>
		    <br />
			...
			<select name = "iblock_sku_id" class="sku_detail">
			    <?foreach ($arIBlocks as $key=>$value):?>
				<option value="<?=$key?>"><?=$value["name"]?></option>		<!-- $value changed to  $value["name"] -->
			    <?endforeach?>
			</select>
			<br />
			<br />
	    ...
	    <?}?>
	    <br />
	    <a href = "/bitrix/admin/mcart_xls_start.php"><input type="button" value="<?=GetMessage("STEP_BACK")?>" /></a>
