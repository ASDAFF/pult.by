...
<script type="text/javascript">
    ...
    function AddCondition(alfavit, titleRow, propCode, propName, modifytypeCode, modifyTypeName, modifySubCode, modifySubName)
    {
        ...
        //newCell.id = "id_row_value_" + b_log_counter;
        //   newCell.align="right";
        newCell.innerHTML = '<a href="#" onclick="MCARTXLSDeleteRow(' + b_log_counter + '); return false;"><input type="button" value="<?=GetMessage("MCART_DELETE_ROW") ?>" /></a>';
...
$format_file = COption::GetOptionString("mcart.xls", "format");			//Get file type
...
if ($format_file == 'Excel2007') {
    ...
    if (empty($inputFileName))
        $errMess = $errMess.GetMessage("XLS_NULL_FILE");
}
...
if (empty($errMess))				//If there are no mistakes, we continue...
{
    ...
    <form action="/bitrix/admin/mcart_xls_import_step_2.php"  method="POST">
        ...
        try {
            $inputFileType = PHPExcel_IOFactory::identify($inputFileName);	//Get file type
	    //
	...
	{    
	    ...
	    // Get list IBlock Section ( получаем список разделов ИБ, в который будем добавлять) =======================
	...
	{	
        ...
        if ($inputFileType == 'Excel2007')
	{?>
	    <!-- Output Iblock Section (select for update element)(выводим вып.списк. разделов ИБ для обновления)=== -->
	    ...
	    <!-- Echo Iblock Section (select for new element)(выводим вып.списк. разделов ИБ для новых записей)======-->
	...
	<!-- ============== Echo tables EXAMPLE ================================== -->  
        ...
        <table border="1">
            ...
            <tr>
                <? foreach ($alfavit as $bukva): ?>
                    <td align="center"><h3><?= $bukva ?></h3></td>
                <? endforeach; ?>
            </tr>
	
            <?
            $alfavit[] = "CONSTANT";
	    //Mассив c именами полей файла
            $ARR_TITLE_ROW = array();						//Cls array
                ...
                for ($row = $firstRow; $row <= ($firstRow + 5); $row++)
		{
 	    ...
 	    //============================= END (IF file type CSV)===============================================
	    ?>

            <?	// IF file type Excel2007
		//( Если файл XLS, то читаем заголовки его полей и выводим в таблицу c примером - первое поле )
	    if ($inputFileType == 'Excel2007') 
	    {
                $rowTitleData = $sheet->rangeToArray($firstColumn . $titleRow . ':' . $highestColumn . $titleRow, NULL, TRUE, FALSE);
                foreach ($rowTitleData as $row_str)
	...
	<!-- ==== IF file type Excel2007 ==== -->
		...
		$arMyFields = CIBlock::getFields($XLS_IBLOCK_ID);

		/*$SrcPropID['NAME']		    = $arMyFields['NAME']['NAME'];*/
		$SrcPropID['ACTIVE']		    = $arMyFields['ACTIVE']['NAME'];
		$SrcPropID['ACTIVE_FROM']	    = $arMyFields['ACTIVE_FROM']['NAME'];
		$SrcPropID['ACTIVE_TO']		    = $arMyFields['ACTIVE_TO']['NAME'];
		$SrcPropID['SORT']		    = $arMyFields['SORT']['NAME'];
		$SrcPropID['PREVIEW_PICTURE']	    = $arMyFields['PREVIEW_PICTURE']['NAME'];
		$SrcPropID['PREVIEW_TEXT']	    = $arMyFields['PREVIEW_TEXT']['NAME'];
		$SrcPropID['DETAIL_PICTURE']	    = $arMyFields['DETAIL_PICTURE']['NAME'];
		$SrcPropID['DETAIL_TEXT']	    = $arMyFields['DETAIL_TEXT']['NAME'];
		$SrcPropID['CODE']		    = $arMyFields['CODE']['NAME'];
		$SrcPropID['TAGS']		    = $arMyFields['TAGS']['NAME'];
		//$SrcPropID['XML_ID']		    = $arMyFields['XML_ID']['NAME'];
			...
			<select name = "XLS_IDENTIFY[modify_subaction]" >
                ...
                foreach ($property_data as $property):
				    ...
				    <? endforeach; ?>	
                        ...
                        <td>
                            <a href="javascript:void(0);" onclick="MCARTXLSDeleteRow(<?= $i ?>);return false;">
				<?= GetMessage("MCART_DELETE_ROW") ?>
			    </a>
                        </td>
                    </tr>
                    <? $i++;
	...
	<?if ($inputFileType == 'CSV') { ?>
		...
		<td>
		    ...
		    <?	endforeach; ?>	
                ...
                <td>
		    <!-- Delete row-->
                    <a href="javascript:void(0);" onclick="MCARTXLSDeleteRow(<?= $i ?>);  return false;"><?=GetMessage("MCART_DELETE_ROW") ?></a>
                </td>
            </tr>
        <? $i++;
	    endforeach;
	...
	<br />
	    ...
	    );return false;"><input type="button" value="<?= GetMessage("ADD_CONDITION"); ?>" /></a>
        <br />
        <br />
	...
	{ ?>
            <h4>
		<input type="checkbox" class="make_translit_code"  name="make_translit_code" value="Y" <? if ($ARR_REAL_PROFILE['need_translit']) echo "checked='checked'" ?>">
                <?= GetMessage("MAKE_TRANSLIT_CODE") ?>
	    </h4>
            <br />
            <br />
            <h4>
		<input type="checkbox" class="save_profile"  name="save_profile" value="Y"><?= GetMessage("XLS_SAVE_PROFILE") ?>
	    </h4>
            <br />
    ...
    <? } ?>	
        ...
        <a href = "/bitrix/admin/mcart_xls_import.php"><input type="button" value="<?=GetMessage("STEP_BACK")?>" /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	CAdminMessage::ShowMessage($errMess); ?>
